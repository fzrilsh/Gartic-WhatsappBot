    ...
    open(msg){
        ...
        this.reqplayersTm = setTimeout(async() => {
            ...
            if(gartic.players.length<1){
                ...
                setTimeout(() => {
                    gartic.client.sendMessage(gartic.id, `[/] `)
                    gartic.newSection()
                }, 5000);
            }
        }, 30000)
        ...
        this.client.on('message_reaction', async(react) => {
            if(react.msgId.id == msg.id && gartic.reqplayers && !gartic.players.find(v => v == react.senderId)){
                ...
                gartic.client.sendMessage(gartic.id, `[+] _@${react.senderId.split('@')[0]}_ ready for draw and guess`, { mentions: [await this.client.getContactById(react.senderId)] })
            }
        })
    }
    ...
    setDraw(cb){
        ...
        this.client.on('message', async(message) => {
            ...
            if (this.game.theme.timer && message.hasMedia) {
                ...
                this.client.sendMessage(this.playersQueue, media, { caption: "You have selected object: *"+selectedButtonId.toUpperCase()+"*\nNow please draw a *"+selectedButtonId.toUpperCase()+"* and send it here, you have 1 minute for draw it" })
